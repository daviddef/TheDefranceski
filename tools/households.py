...
CANON = {
 ...
 'antonio':  'antonio antonius antonium antonii anto anton antonia antonium',
 'matteo':   'matteo mattio matio mattheus mathaeus mathæus mathæi matheus mattia matthia mateo',
 'francesco':'francesco franciscus francisci franciscum franco franz francescho francisca franciscae franciscæ franca',
 'pietro':   'pietro pier piero pieto petrus petrum petri',
 'valentino':'valentino valentinus valentini valentinum valentio valentin',
 'giuseppe': 'giuseppe josephus josephi josepham joseph josef josepha giuseppa',
 ...
 'domenico': 'domenico dominicus domco domenica dominica domca domča domenicae',
 'gasparo':  'gasparo gaspero gaspo gaspar gasparus gasparina',
 'nicolo':   'nicolo niccolo nicolaus nicolaa nicoló nicolò nicolaus',
 'vittorio': 'vittorio victorius vitorio victorii vitorii vittoria victoria',
 ...
 'giacomo':  'giacomo giacoma jacobus jacobina giacomo',
...
FEMALE = {'maria','elena','orsola','caterina','lucia','anna','stella','agata','eufemia','teresa','domenica',
          ...
          'maddalena','filomena','veneranda','pasqua','fioretta','colotta','bonetta','zacchera'}
...
def build(rows):
    houses = defaultdict(lambda: {'father': None, 'mother': None, 'children': [], 'places': defaultdict(int), 'years': []})
    for r in rows:
        ...
        place = (r['place'] or '').split(',')[0].strip()
        key = (frozenset((fk, mk)), place)
        h = houses[key]
        ...
        if not h['father']: h['father'] = dad[1]
        if not h['mother']: h['mother'] = mum[1]
        ...
        if r['place']:
            h['places'][r['place'].split(',')[0].strip()] += 1
        if y:
            h['years'].append(y)
    out = []
    for (keyset, _place), h in houses.items():
        ...
        out.append({'key': ' + '.join(sorted(keyset)), 'father': h['father'], 'mother': h['mother'],
                    'place': place, 'from': min(h['years']) if h['years'] else None,
    ...
    for h in sorted(out, key=lambda x: (x['key'], x['from'] or 9999)):
        hit = None
        for m2 in merged:
            if m2['key'] != h['key'] or m2['place'] != h['place']:
                continue
            a = (m2['mother'] or '').lower().split()[-1]
            b = (h['mother'] or '').lower().split()[-1]
            if lev(a, b) <= 2:
                hit = m2; break
        if hit:
    ...
    merged.sort(key=lambda h: (-h['n'], h['from'] or 9999))
    return merged

if __name__ == '__main__':
    rows = load('data/familysearch-croatia.csv') + load('data/familysearch-austria.csv')
    hs = build(rows)
